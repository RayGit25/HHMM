...
var anchos = [13, 8, 15, 8, 8, 10, 10, 5, 8, 10];
...
function listarInicial(rpta) {
    ...
    if (rpta != "") {
        ...
        //listarTipoAdmision();
        //listarPeriodo();
        var listaMensaje = listaGeneral[4].split("¯");

        for (var i = 0; i < listaMensaje.length; i++) {
            var m = listaMensaje[i].split("¦");
            matrizMensaje.push(m[1]);
        }
...
function configurarControles() {
            ...
            if (matrizDetalle.length > 0 && MedicosSeleccionados.length > 0) {
                ...
                if (SeleccionActualProceso == "P" || SeleccionActualProceso == "O") {
                    if (SeleccionActualProceso == "P") {
                        ...
                        mensaje = matrizMensaje[0]; // "¿Está seguro de generar la obligación de los registros seleccionados en el SPRING ?";
                        titulo = matrizMensaje[1]; // "INTEGRACIÓN CON SPRING";
                    }
                        ...
                        for (var y = 0; y < MedicosSeleccionados.length; y++) {
                            for (var x = 0; x < listaProvision.length; x++) {
                                ...
                                if (MedicosSeleccionados[y] == (valor[0] * 1)) {
                                    ...
                                    MatrizMedicosSeleccionados[contador][2] = valor[8]; //Tipo Documento
                                    ...
                                    MatrizMedicosSeleccionados[contador][7] = valor[11]; //PersonaId
                                    MatrizMedicosSeleccionados[contador][8] = valor[12]; //CMP
                                    MatrizMedicosSeleccionados[contador][9] = valor[6]; // Total
                                    contador = contador + 1;
                                    break;
                                }
                            }
    ...
    let btnCerrarModalUnificacion = document.getElementById('btn_PPL_MdlUP_Cancelar');
    btnCerrarModalUnificacion.onclick = function () {
        abrirPopup('PopupUnificacionPlanilla');
    };

    let btn_PPL_MdlUP_UnificarPlantillas = document.getElementById("btn_PPL_MdlUP_UnificarPlantillas");
    if (btn_PPL_MdlUP_UnificarPlantillas != null) {
        btn_PPL_MdlUP_UnificarPlantillas.onclick = function () {
            var frameTmp = document.getElementById('divifrPlanillaUnificacion');
            let registrosSeleccionados = frameTmp.contentWindow._registrosSeleccionados ? frameTmp.contentWindow._registrosSeleccionados : 0;
            if (registrosSeleccionados.length > 0) {
                var pos1 = window.location.href.indexOf("Proceso");
                urlBase = window.location.href.substring(0, pos1);
                ss = window.parent.document.getElementById("iss").value;
                let data = document.getElementById('txt_PPL_MdlUP_Planilla').value; //[0] idPlanillaUnificada
                data += '¯' + registrosSeleccionados.join('¦'); //[1] lstIdPlanillasUnificar
                data += '¯' + document.getElementById('txt_PPL_MdlUP_Motivo').value; //[2] Motivo
                var url = urlBase + "Proceso/unificarPlanillas/?ss=" + ss;
                $.ajax(url, "post", mostrarRepuestaUnificacion, data);
            }
            else {
                mostraralerta('Seleccione al menos una planilla');
            }
        }
    }

    let btn_PPL_MdlCE_Cancelar = document.getElementById('btn_PPL_MdlCE_Cancelar');
    btn_PPL_MdlCE_Cancelar.onclick = function () {
        abrirPopup('PopupComprobanteElectronico');
    };

    let btn_PPL_MdlCE_Grabar = document.getElementById("btn_PPL_MdlCE_Grabar");
    if (btn_PPL_MdlCE_Grabar != null) {
        btn_PPL_MdlCE_Grabar.onclick = function () {
            var frameTmp = document.getElementById('divifrComprobanteElectronico');
            let registrosSeleccionados = frameTmp.contentWindow._registrosSeleccionados ? frameTmp.contentWindow._registrosSeleccionados : 0;
            if (registrosSeleccionados.length > 0) {
                let aData = registrosSeleccionados[0].split('¦'); //[0]Serie¦[1]Numero¦[2]FechaEmision
                let idPlanilla = document.getElementById("hdf_PPL_MdlCE_PlanillaId").value;
                let idPersona = document.getElementById("hdf_PPL_MdlCE_PersonaId").value;
                for (x = 0; x < MatrizFiltroMedicosSeleccionados.length; x++) {
                    if (MatrizFiltroMedicosSeleccionados[x][0] == idPlanilla) {// && MatrizFiltroMedicosSeleccionados[x][7] == idPersona) {
                        MatrizFiltroMedicosSeleccionados[x][4] = aData[0]; //Serie
                        MatrizFiltroMedicosSeleccionados[x][3] = aData[1]; //Numero
                        MatrizFiltroMedicosSeleccionados[x][5] = aData[2]; //FechaEmision
                        break;
                    }
                }
                for (x = 0; x < MatrizMedicosSeleccionados.length; x++) {
                    if (MatrizMedicosSeleccionados[x][0] == idPlanilla) {// && MatrizMedicosSeleccionados[x][7] == idPersona) {
                        MatrizMedicosSeleccionados[x][4] = aData[0]; //Serie
                        MatrizMedicosSeleccionados[x][3] = aData[1]; //Numero
                        MatrizMedicosSeleccionados[x][5] = aData[2]; //FechaEmision
                        break;
                    }
                }
                let tBodyFilas = document.getElementById('tbConfirmarProvision').children;
                let aCelda;
                for (let x = 0; x < tBodyFilas.length; x++) {
                    if (tBodyFilas[x].getAttribute('data-id') == idPlanilla) {
                        aCelda = tBodyFilas[x].children;
                        aCelda[2].children[0].value = aData[0]; //Serie
                        aCelda[3].children[0].value = aData[1]; //Numero
                        aCelda[4].children[0].value = aData[2]; //FechaEmision
                        break;
                    }
                }
                abrirPopup("PopupComprobanteElectronico");
                ...
                mostraralerta('Seleccione al menos un registro');
            }
        }
    }
}
...
function crearCabeceraExportar() {
    ...
    cabecera += "<td style='width: 250px'>Sucursal</td>";
    cabecera += "<td style='width: 100px'>Nro Proceso</td>";
    cabecera += "<td style='width: 300px'>Descripción</td>";
    cabecera += "<td style='width: 100px'>Tipo</td>";
    cabecera += "<td style='width: 100px'>Periodo</td>";
    cabecera += "<td style='width: 100px'>Fecha Inicio</td>";
    cabecera += "<td style='width: 100px'>Fecha Final</td>";
    cabecera += "<td style='width: 100px'>Cantidad</td>";
    cabecera += "<td style='width: 100px'>Monto</td>";
    cabecera += "<td style='width: 150px'>Estado Planilla</td>";
    cabecera += "</tr>";
    return cabecera;
}
            ...
            for (var j = 0; j < nCampos; j++) {
                if (j != 0) {
                    if (j == 10) {
                        ...
                        contenido += "<td align='center'>" + estado + "</td>";
                    } else if (j == 4) {
                        contenido += "<td align='center'>" + (matriz[i][j] == "1" ? "NORMAL" : "REGULARIZACIÓN") + "</td>";
                    }
                    else {
                        contenido += "<td>" + matriz[i][j] + "</td>";
                    }
                }
            }
            contenido +="</tr>";
...
function exportacionDetalle(objeto) {
    ...
    if (nRegistros > 0) {
        ...
        contenido += "</table></html>";
        var formBlob = new Blob([contenido], { type: 'application/vnd.ms-excel' });
        objeto.download = "ProcesoPlanillaDetalle.xls";
        objeto.href = window.URL.createObjectURL(formBlob);
    }
}
...
function RptaCalculo(rpta) {
    ...
    if (cEnvios < nEnvios) {
                ...
                if (matrizDetalle.length > 0 && MedicosSeleccionados.length > 0) {
                    ...
                    if (SeleccionActualProceso == "P" || SeleccionActualProceso == "O") {
                        if (SeleccionActualProceso == "P") {
                            ...
                            mensaje = matrizMensaje[2]; // "¿Está seguro de generar la obligación de los registros seleccionados en el SPRING ?";
                            titulo = matrizMensaje[3]; // "INTEGRACIÓN CON SPRING";
                        }
        ...
        if (rpta != "-1" && (rpta.indexOf("NEXT|-1") == -1)) {
            ...
            switch (SeleccionActualProceso) {
                case "G":
                    mostraralerta(matrizMensaje[4]); // "Obligación debe ser revertido primero en el SPRING"
                    break;
                case "F":
                    mostraralerta(matrizMensaje[5]); //"Obligación debe ser revertido primero en el SPRING"
                    break;
                default:
    ...
    if (opcion) {
        ...
        for (var j = 0; j < nCampos; j++) {
            ...
            if (j == 3) {
                ...
                contenido += "<option value='1'>NORMAL</option>";
                contenido += "<option value='2'>REGULARIZACIÓN</option>";
                contenido += "</select>";
            } else {
                if (j == 9) {
                    contenido += "<select class='Combo' name='cabecera' style='width:90%'><option value=''>TODOS</option>";
...
function mostrarMatrizMedicosConfirmar(indicePagina) {
    ...
    if (n > 0) {
        ...
        var fin = inicio + 8;
        var contador = 0;
        let planillaId, nombreCompletoMedico, personaId, cmp, tipoDocumento, monto;
        let data;
        for (var z = 0; z < n; z++) {
            if (z < n) {
                planillaId = MatrizMedicosSeleccionados[z][0];
                nombreCompletoMedico = MatrizMedicosSeleccionados[z][1];
                personaId = MatrizMedicosSeleccionados[z][7];
                cmp = MatrizMedicosSeleccionados[z][8];
                tipoDocumento = MatrizMedicosSeleccionados[z][2];
                monto = MatrizMedicosSeleccionados[z][9];
                data = nombreCompletoMedico + "¦" + cmp + "¦" + planillaId;
                contenido += "<tr class='FilaDatos trMedConf' data-id='";
                contenido += MatrizMedicosSeleccionados[z][0];
                contenido += "'><td>";
                contenido += nombreCompletoMedico;
                contenido += "</td><td>"
                ...
                contenido += "<span class='Icons fa-eye MessMed' style='display:none'></span>";
                contenido += "<span class='Icons fa-search puntero";
                contenido += "' onclick='mostrarSeleccionComprobanteElectronico(";
                contenido += personaId;
                contenido += ",\"";
                contenido += monto;
                contenido += "\",\"";
                contenido += tipoDocumento;
                contenido += "\",\"";
                contenido += data;
                contenido += "\")'></span>";
                contenido += "</td>";
...
function mostrarUnificacionPlanilla(id, dato) {
    var ifrPlanillaDetalleConceptosOA = document.getElementById("ifrPlanillaUnificacion");
    if (ifrPlanillaDetalleConceptosOA.innerHTML == "") {
        ifrPlanillaDetalleConceptosOA.innerHTML = "<iframe id='divifrPlanillaUnificacion' style='margin:0;padding:0;width:100%;height:400px;border: 1px solid transparent;' src='" + urlBase + "Proceso/ProcesoUnificacionPlanilla/?ss=" + ss + "&data=" + id + "'></iframe>";
    }
    else {
        document.getElementById('divifrPlanillaUnificacion').src = urlBase + "Proceso/ProcesoUnificacionPlanilla/?ss=" + ss + "&data=" + id;
    }
    document.getElementById('divifrPlanillaUnificacion').style.height = (document.getElementById("div_PPL_MdlUP_Contenedor").clientHeight - 150) + "px";

    document.getElementById("txt_PPL_MdlUP_Motivo").value = '';
    document.getElementById("txt_PPL_MdlUP_Planilla").value = id;
    document.getElementById("txt_PPL_MdlUP_Medico").value = dato;
    abrirPopup("PopupUnificacionPlanilla");
}

function mostrarRepuestaUnificacion(rpta) {
    if (rpta != 'OK') {
    ...
    document.getElementById('btnDetalleCancelar').click(); //Para volver a la vista anterior
}

function mostrarSeleccionComprobanteElectronico(id, monto, tipoDocumento, data) {
    let aData = data.split('¦');
    let _data = id + '|' + monto + '|' + tipoDocumento;
    var ifrPlanillaDetalleConceptosOA = document.getElementById("ifrComprobanteElectronico");
    if (ifrPlanillaDetalleConceptosOA.innerHTML == "") {
        ifrPlanillaDetalleConceptosOA.innerHTML = "<iframe id='divifrComprobanteElectronico' style='margin:0;padding:0;width:100%;height:400px;border: 1px solid transparent;' src='" + urlBase + "Proceso/ComprobanteElectronico/?ss=" + ss + "&data=" + _data + "'></iframe>";
    }
    else {
        document.getElementById('divifrComprobanteElectronico').src = urlBase + "Proceso/ComprobanteElectronico/?ss=" + ss + "&data=" + _data;
    }
    document.getElementById('divifrComprobanteElectronico').style.height = (document.getElementById("div_PPL_MdlCE_Contenedor").clientHeight - 220) + "px";

    document.getElementById("hdf_PPL_MdlCE_PersonaId").value = id;
    document.getElementById("hdf_PPL_MdlCE_CMP").value = aData[1];
    document.getElementById("hdf_PPL_MdlCE_PlanillaId").value = aData[2];
    document.getElementById("txt_PPL_MdlCE_Medico").value = aData[0];
    var descripcionTipoDocumento = "";
    for (var k = 0; k < listaTipoDocumento.length; k++) {
        var campos = listaTipoDocumento[k].split("¦");
        if (campos[0] === tipoDocumento) {
            descripcionTipoDocumento = campos[1];
            break;
        }
    }
    document.getElementById("txt_PPL_MdlCE_TipoDocumento").value = descripcionTipoDocumento;
    abrirPopup("PopupComprobanteElectronico");
}